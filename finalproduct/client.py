...
def imgDown():    
    ...
    while True:
        ftp_client.get(remotepath+"/"+imagename, imagename);
        clock.tick(fps);
        
def controlsUp(string):
    file = open(controlsname, "w+")
    file.write(string)
    ftp_client.put(controlsname, remotepath + "/" + controlsname);
    

if __name__=='__main__':
    sio = socketio.Client()
    sio.connect('http://1920.lakeside-cs.org:6544/')
    @sio.on("image message")
    def image(data):
        global videoImg
        videoImg = pygame.image.fromstring(data, (160,120), "RGB")
    ...
    def button(x,y):
        ...
        return down;
    # the loop
    download = Process(target = imgDown)
    download.start()
    while not crashed:    
        # gets all the events
        ...
        sio.emit('controls message', str(sendArray[0])+str(sendArray[1])+str(sendArray[2])+str(sendArray[3]))
        #print(localpath);
        try:
        ...
        s(xS,yS,sDown);
        d(xD,yD,dDown);
        videoImg = pygame.transform.scale(videoImg, (160*4, 120*4))
        videoImage(300,50,videoImg);
        #videoImage()
        #again testing VVV
        #updates the screen
